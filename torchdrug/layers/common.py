...
class MultiLayerPerceptron(nn.Module):
    ...
    Note there is no batch normalization, activation or dropout in the last layers.
...
class PairNorm(nn.Module):
    """
    Pair normalization layers proposed in `PairNorm: Tackling Oversmoothing in GNNs`_.
...
class InstanceNorm(nn.modules.instancenorm._InstanceNorm):
    """
    Instance normalization for graphs. This layers follows the definition in
...
class Sequential(nn.Sequential):
    ...
    Compared to the vanilla nn.Sequential, this layers additionally supports the following features.

    1. Multiple input / output arguments.

    >>> # layer1 signature: (...) -> (a, b)
    >>> # layer2 signature: (a, b) -> (...)
    >>> layers = layers.Sequential(layer1, layer2)

    2. Global arguments.

    >>> # layer1 signature: (graph, a) -> b
    >>> # layer2 signature: (graph, b) -> c
    >>> layers = layers.Sequential(layer1, layer2, global_args=("graph",))

    Note the global arguments don't need to be present in every layers.
    ...
    >>> layers = layers.Sequential(layer1, layer2, allow_unused=True)

    When dict outputs are used with global arguments, the global arguments can be explicitly
    overwritten by any layers outputs.
